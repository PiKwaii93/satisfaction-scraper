...
Le tag du run `production_alias_promotion_status=not_attempted` décrit l'entraînement candidat initial. L'alias `production` constaté sur v53 le 24 septembre 2026 provient de la promotion ultérieure : ces deux éléments ne se contredisent pas.
...
Aucun réentraînement, changement d'alias, écriture MLflow/PostgreSQL ni accès à la VM n'a été effectué pour constituer ce dossier.
